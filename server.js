...
app.prepare().then(() => {
  ...
  io.on("connection", (socket) => {
      ...
      if (!wasOnline) {
        ...
        if (undeliveredGroupMessages.size > 0) {
          ...
          for (const [
            roomId,
            userMessages,
          ] of undeliveredGroupMessages.entries()) {
            if (userMessages.has(userId)) {
              const messages = userMessages.get(userId);
              console.log(
                `📦 Found ${messages.length} undelivered group messages for user ${userId} in room ${roomId}`,
              );
    ...
    socket.on("join-chat", ({ roomId, userId, groupMembers: members }) => {
      ...
      if (undeliveredGroupMessages.has(roomId)) {
        ...
        if (userMessages.has(userId)) {
          const messages = userMessages.get(userId);
          console.log(
            `📦 Delivering ${messages.length} group messages to user ${userId} in room ${roomId}`,
          );
    ...
    socket.on("member-joined", (data) => {
      ...
      console.log(
        `✅ Member join notification broadcast to room ${data.roomId}`,
      );
    });
    ...
    socket.on("send-message", (message) => {
      ...
      console.log(
        `From: ${message.senderId}, Room: ${message.roomId}, Is Group: ${message.isGroupMessage}`,
      );
      ...
      if (message.isGroupMessage) {
        ...
        console.log(
          `📊 Group members: ${onlineMembers.size} online, ${offlineMembers.size} offline`,
        );
        ...
        if (offlineMembers.size > 0) {
          console.log(
            `📦 Storing message for ${offlineMembers.size} offline members`,
          );
        ...
        if (receiverSocketId && receiverOnline) {
          console.log(
            `📤 Sending message to online user: ${message.receiverId}`,
          );
          ...
          console.log(
            `📦 Storing message for offline user: ${message.receiverId}`,
          );
    ...
    socket.on(
      "get-undelivered-status",
      ({ roomId, userId, isGroupMessage }) => {
        if (isGroupMessage) {
          if (undeliveredGroupMessages.has(roomId)) {
            const userMessages = undeliveredGroupMessages.get(roomId);
            if (userMessages.has(userId)) {
    ...
    socket.on("disconnect", () => {
      ...
      if (userInfo) {
        ...
        socket
          .to(roomId)
          .emit("user-online", { userId, online: false, lastSeen });
