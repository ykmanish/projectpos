...
export default function AttachmentPreviewModal({ isOpen, onClose, attachment, onNext, onPrevious, hasNext, hasPrevious }) {
  ...
  return (
    <>
      <div 
        className="fixed inset-0 bg-black/80 dark:bg-black/90 z-50 transition-opacity"
        onClick={onClose}
      />
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        >
          {/* Header */}
          <div className="absolute top-0 left-0 right-0 z-10 flex items-center justify-between p-4 bg-gradient-to-b from-black/80 via-black/50 to-transparent dark:from-black/90 dark:via-black/60">
            <div className="flex items-center gap-3">
              <button
                onClick={onClose}
                className="p-2 bg-black/50 hover:bg-black/70 dark:bg-black/70 dark:hover:bg-black/90 rounded-full text-white transition-colors backdrop-blur-sm"
              >
                <X size={20} />
              </button>
          ...
          {!attachment.preview && hasPrevious && (
            <button
              onClick={onPrevious}
              className="absolute left-4 top-1/2 -translate-y-1/2 z-10 p-3 bg-black/50 hover:bg-black/70 dark:bg-black/70 dark:hover:bg-black/90 rounded-full text-white transition-colors backdrop-blur-sm"
            >
              <ChevronLeft size={24} />
            </button>
          )}
          {!attachment.preview && hasNext && (
            <button
              onClick={onNext}
              className="absolute right-4 top-1/2 -translate-y-1/2 z-10 p-3 bg-black/50 hover:bg-black/70 dark:bg-black/70 dark:hover:bg-black/90 rounded-full text-white transition-colors backdrop-blur-sm"
            >
          ...
          <div className="flex-1 flex items-center justify-center p-8">
            {attachment.type === 'image' ? (
              ...
              <video
                ...
                controls
                autoPlay
                className="max-w-full max-h-[80vh] rounded-lg shadow-2xl dark:shadow-black/50"
